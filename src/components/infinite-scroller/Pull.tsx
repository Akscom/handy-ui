...
export default class Pull extends PureComponent<PullProps, any> {
  ...
  private throttledScroll:any;

  private wrapTouchstartY:any;
  ...
  wrapTouchstart = (event:any) => {
    const touch = event.touches[0];
    this.wrapTouchstartY = touch.pageY;
  };

  wrapTouchmove = (event:any) => {
    const touch = event.touches[0];
  ...
  onDragMove = (event:any, offsetY:any) => {
  ...
  onDragEnd = (_event:any, offsetY:any) => {
    // 没有产生位移
    if (!offsetY) {
      return;
    }
   ...
   */
  doTransition = (offsetY:any, animationDuration:any) => {
   ...
   */
  doRefreshAction = (refreshState:any, offsetY?: number) => {
